...
import dev.virtualpatch.patch.IntentSanitizer;
...
public final class VirtualCore {
    ...
    public void startup(Context context, SettingConfig config) throws Throwable {
        if (!isStartUp) {
            ...
            // NativeEngine.dynamicHooks(getContext().getCacheDir(), "nopackages.apk", "dev.virtualpatch.patch.Patch");
            ...
            Trace.beginSection("preloadPatches");
            NativeEngine.preloadPatches(context.getCacheDir());
            NativeEngine.preloadNativePatches(context.getCacheDir());
            Trace.endSection();

            Trace.beginSection("DynamicProxyPatch");
            NativeEngine.applyDynamicProxyPatch(getContext().getCacheDir());
            Trace.endSection();
