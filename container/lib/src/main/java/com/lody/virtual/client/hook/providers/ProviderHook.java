...
public class ProviderHook implements InvocationHandler {
    ...
    public static void addProvider(String authority, HookFetcher fetcher) {
        if (!PROVIDER_MAP.containsKey(authority)) {
            PROVIDER_MAP.put(authority, new ArrayList<>());
        }
        List<HookFetcher> l = PROVIDER_MAP.get(authority);
        l.add(fetcher);
    }

    public static void addProvider(String authority, Method m) {
        addProvider(authority, new MethodHookFetcher(m));
    }

    private static class MethodHookFetcher implements HookFetcher {

        Method m_method;

        MethodHookFetcher(Method m) {
            m_method = m;
        }
            ...
            if(external) {
                return new ExternalProviderHook(provider) {
                    ...
                    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                        Object ret = m_method.invoke(null, proxy, method, args);
                        if(ret == null) {
    ...
    private static final Map<String, List<HookFetcher>> PROVIDER_MAP = new HashMap<>();

    static {
        addProvider("settings", new HookFetcher() {
            @Override
            public ProviderHook fetch(boolean external, IInterface provider) {
                return new SettingsProviderHook(provider);
            }
        });
        addProvider("downloads", new HookFetcher() {
            @Override
            public ProviderHook fetch(boolean external, IInterface provider) {
                return new DownloadProviderHook(provider);
            }
        });
        addProvider("com.android.badge", new HookFetcher() {
            @Override
            public ProviderHook fetch(boolean external, IInterface provider) {
                return new BadgeProviderHook(provider);
            }
        });
        addProvider("com.huawei.android.launcher.settings", new HookFetcher() {
            @Override
            public ProviderHook fetch(boolean external, IInterface provider) {
                return new BadgeProviderHook(provider);
            }
        });
        addProvider("com.android.externalstorage.documents", new HookFetcher() {
            @Override
            public ProviderHook fetch(boolean external, IInterface provider) {
    ...
    private static class ChainFetcher implements HookFetcher {
        List<HookFetcher> m_fetchers;

        ChainFetcher(List<HookFetcher> f) {
            m_fetchers = f;
        }

        @Override
        public ProviderHook fetch(boolean external, IInterface provider) {
            ProviderHook ret = null;
            for (HookFetcher f: m_fetchers) {
                ret = f.fetch(external, provider);
                provider = createProxy(provider, ret);
            }
            return ret;
        }
    }
    ...
    private static HookFetcher fetchHook(String authority) {
        ...
        List<HookFetcher> fetchers = PROVIDER_MAP.get(authority);
        if (fetchers == null) {
            HookFetcher fetcher = new HookFetcher() {
                @Override
                public ProviderHook fetch(boolean external, IInterface provider) {
                    if (external) {
                        return new ExternalProviderHook(provider);
        ...
        return new ChainFetcher(fetchers);
    }
