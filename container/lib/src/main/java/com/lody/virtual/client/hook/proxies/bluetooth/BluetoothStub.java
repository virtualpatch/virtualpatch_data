...
import mirror.android.bluetooth.BluetoothDevice;
...
public class BluetoothStub extends MethodInvocationProxy<MethodInvocationStub<IInterface>>  {
    ...
    public BluetoothStub() {
        super(new MethodInvocationStub<>(BluetoothDevice.getService.call()));
    }

    @Override
    protected void onBindMethods() {
        Log.d("BT_BIND", "binding bt methods");
        super.onBindMethods();
    ...
    public void inject() throws Throwable {
        final IInterface hookedBM = getInvocationStub().getProxyInterface();
        final IInterface baseBM = getInvocationStub().getBaseInterface();
        Log.d("inject", "hooked: " + hookedBM + " base: " + baseBM);
        BluetoothDevice.sService.set(hookedBM);
        BinderInvocationStub sHookBinder = new BinderInvocationStub(getInvocationStub().getBaseInterface());
        sHookBinder.copyMethodProxies(getInvocationStub());
        sHookBinder.replaceService(SERVER_NAME);
    }

    @Override
    public boolean isEnvBad() {
        return getInvocationStub().getProxyInterface() != BluetoothDevice.sService.get();
    }
