...
#include <Foundation/MinikinHook.h>
...
static void jni_nativeHookMinikin(JNIEnv *env, jclass jclazz) {
    startMinikinHook();
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *_vm, void *) {
    ...
    static JNINativeMethod methods[] = {
            ...
            {"nativeMinikinHook",                  "()V",                                                             (void *) jni_nativeHookMinikin},
    };

    if (env->RegisterNatives(nativeEngineClass, methods, 20) < 0) {
        return JNI_ERR;
    }
